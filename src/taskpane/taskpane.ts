...
async function getTransactions(): Promise<Map<string, Transaction>> {
  ...
  await Excel.run(async context => {
    let sheet = context.workbook.worksheets.getItem("Transactions").getRangeByIndexes(0, 0, 1, txnShape["columnCount"]);
    sheet.load();
    await context
      .sync()
      .then(() => {
      ...
      .then(async () => {
        ...
        for (let i = 1; i < txnShape["rowCount"]; i += step) {
          let sheet = context.workbook.worksheets
            ...
            .getRangeByIndexes(i, 0, step, txnShape["columnCount"])
